...
(function(){
  ...
  s.textContent='@keyframes isle-step{to{background-position-x:calc(-100% - 0px)}}';
  document.head.appendChild(s);
})();

// Build a sprite-animated div via real DOM (safe for data URIs)
function mkSprEl(sheet, d, dispW, dispH, fps){
  let el=document.createElement('div');
  el.style.cssText=`position:absolute;image-rendering:pixelated;`
    +`width:${dispW}px;height:${dispH}px;`
    +`background-image:url('${sheet}');`
    +`background-size:${d.n*dispW}px ${dispH}px;`
    +`background-position:0 0;background-repeat:no-repeat;`
    +`animation:isle-step ${(d.n/fps).toFixed(2)}s steps(${d.n},end) infinite;`;
  return el;
}

// Build a static image div via real DOM
function mkImgEl(src, w, h){
  let el=document.createElement('div');
  el.style.cssText=`position:absolute;image-rendering:pixelated;`
...
// Build a complete island and append its children to a container el
// Returns the container el (already in DOM when this runs)
function buildIsle(containerEl, cfg){
  let {w=180, dec='tree', decW=70, decH=80, decOffX=0, decOffY=0, waterfall=false}=cfg;
  let bodyH  = Math.round(w * 0.62);
  let totalH = bodyH + Math.round(w * 0.20);
  let rimY   = Math.round(bodyH * 0.21); // top of teal pool rim

  containerEl.style.cssText += `position:relative;width:${w}px;height:${totalH}px;display:inline-block;`;

  // ── Body sprite (stretched wide) ────────────────────────────────
  let body = mkImgEl(_A.body, w, bodyH);
  body.style.left='0'; body.style.top='0';
  containerEl.appendChild(body);

  // ── Waterfall (Lernen + Gemeinschaft) ───────────────────────────
  if(waterfall){
    let wfDispW = Math.round(w * 0.75);
    let wfDispH = Math.round(wfDispW * (_AD.wf.fh / _AD.wf.fw));
    let wf = mkSprEl(_A.wfSheet, _AD.wf, wfDispW, wfDispH, 8);
    wf.style.left = Math.round((w - wfDispW)/2) + 'px';
    wf.style.top  = Math.round(rimY - wfDispH*0.08) + 'px'; // rim overlaps top of wf
    wf.style.zIndex = '2';
    containerEl.appendChild(wf);
  }

  // ── Decoration ──────────────────────────────────────────────────
  if(dec==='tree'){
    let dw=Math.round(w*0.56), dh=Math.round(dw*(_AD.tree.fh/_AD.tree.fw));
    let el=mkSprEl(_A.treeSheet,_AD.tree,dw,dh,8);
    el.style.left=Math.round((w-dw)/2+decOffX)+'px';
    el.style.top=Math.round(rimY-dh+decOffY+2)+'px';
    el.style.zIndex='4';
    containerEl.appendChild(el);
  } else if(dec==='statue'){
    let dw=Math.round(w*0.56), dh=Math.round(dw*(_AD.stat.fh/_AD.stat.fw));
    let el=mkSprEl(_A.statSheet,_AD.stat,dw,dh,4);
    el.style.left=Math.round((w-dw)/2+decOffX)+'px';
    el.style.top=Math.round(rimY-dh+decOffY+4)+'px';
    el.style.zIndex='4';
    containerEl.appendChild(el);
  } else if(dec==='flower'){
    let el=mkImgEl(_A.flower,decW,decH);
    el.style.left=Math.round((w-decW)/2+decOffX)+'px';
    el.style.top=Math.round(rimY-decH+decOffY)+'px';
    el.style.zIndex='4';
    containerEl.appendChild(el);
  } else if(dec==='crystal'){
    let el=mkImgEl(_A.crystal,decW,decH);
    el.style.left=Math.round((w-decW)/2+decOffX)+'px';
    el.style.top=Math.round(rimY-decH+decOffY)+'px';
    el.style.zIndex='4';
    containerEl.appendChild(el);
  ...
  // ── Animated flying rocks (left side) ───────────────────────────
  let rkW=Math.round(w*0.14), rkH=Math.round(rkW*(_AD.rock.fh/_AD.rock.fw));
  let rock=mkSprEl(_A.rockSheet,_AD.rock,rkW,rkH,5);
  rock.style.left=Math.round(-rkW*0.3)+'px';
  rock.style.top=Math.round(bodyH*0.28)+'px';
  rock.style.opacity='0.80';
  containerEl.appendChild(rock);

  // ── Animated cloud wisp below ────────────────────────────────────
  let clW=Math.round(w*0.58), clH=Math.round(clW*(_AD.cloud.fh/_AD.cloud.fw));
  let cloud=mkSprEl(_A.cloudSheet,_AD.cloud,clW,clH,4);
  cloud.style.left=Math.round((w-clW)/2)+'px';
  cloud.style.top=Math.round(bodyH-clH*0.25)+'px';
  cloud.style.opacity='0.45';
  containerEl.appendChild(cloud);

  // ── Drop shadow ──────────────────────────────────────────────────
  let sh=document.createElement('div');
  sh.style.cssText=`position:absolute;left:${Math.round(w*0.30)}px;top:${bodyH+6}px;`
    +`width:${Math.round(w*0.40)}px;height:6px;border-radius:50%;`
...
function rMap(){
  ...
  // Set the map skeleton HTML first (no island content yet)
  c.innerHTML=`
<div class="map-outer">
  ...
  <div class="map-ocean">
    ...
    <div class="map-scatter">
      ...
      <div class="isle isle-lernen" id="isle-lernen" onclick="setTab('studyhome')" style="cursor:pointer">
        <div class="isle-badge-wrap">${due>0?`<div class="isle-badge" style="position:absolute;top:-8px;right:-4px;z-index:10">${due} due</div>`:''}</div>
        <div class="isle-label">Lernen</div>
        <div class="isle-sub">${knownC} known</div>
      </div>

      <div class="isle isle-woerter" id="isle-woerter" onclick="setTab('browse')" style="cursor:pointer">
        <div class="isle-label">Wörter</div>
        <div class="isle-sub">${Object.keys(DATA).length} Kategorien</div>
      </div>

      <div class="isle isle-gemein" id="isle-gemein" onclick="setTab('social')" style="cursor:pointer">
        <div class="isle-label">Gemeinschaft</div>
        <div class="isle-sub">${weekBP} BP this week</div>
      </div>

      <div class="isle isle-planen" id="isle-planen" onclick="setTab('plan')" style="cursor:pointer">
  ...
  // Now build islands via real DOM (data URIs safe here)
  buildIsle(document.getElementById('isle-lernen'),  {w:196, dec:'tree',    waterfall:true});
  buildIsle(document.getElementById('isle-woerter'), {w:155, dec:'flower',  decW:58, decH:66});
  buildIsle(document.getElementById('isle-gemein'),  {w:158, dec:'statue',  waterfall:true});
  buildIsle(document.getElementById('isle-planen'),  {w:130, dec:'crystal', decW:50, decH:62});
